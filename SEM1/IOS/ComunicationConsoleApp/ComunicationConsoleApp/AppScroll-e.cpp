...
void returnsInfo(BYTE baseClass, BYTE subClass, BYTE progInterface) {
	for (int i = 0; i < PCI_CLASS_TABLE_LEN; i++)
	{
			...
			&& PciClassTable[i].ProgIf == progInterface)
		{
			...
			cout << PciClassTable[i].ProgIfDesc;
		}
	}
}

void returnsIds(DWORD vendorId, DWORD deviceId) {
	...
	for (int i = 0; i < 15; i++)
		for (int j = 0; j < 31; j++)
			for (int k = 0; k < 7; k++)
			{
				...
				returnsInfo(baseClass, subClass, progInterface);
				returnsIds(wVendorID, deviceId);
